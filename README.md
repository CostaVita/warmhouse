...
Созданы два микросервиса на языке Go для плавного перехода к микросервисной архитектуре:

1. **Device Registry Service** (`apps/microservices/device-registry/`)
   - Управление устройствами/датчиками (CRUD операции)
   - REST API: `/api/v1/devices`
   - Порт: 8082

2. **Telemetry Service** (`apps/microservices/telemetry-service/`)
   - Управление телеметрией (значения датчиков)
   - REST API: `/api/v1/telemetry`
   - Порт: 8083

**Архитектурные решения:**
- Оба микросервиса используют единую базу данных PostgreSQL для упрощения логики на этапе MVP
- Монолит (`smart_home`) остается точкой входа (триггером) - все внешние запросы идут через него
- Под капотом монолит вызывает микросервисы через REST API, постепенно перенося логику
- Все эндпоинты монолита сохранены для обратной совместимости

**Интеграция:**
- Монолит использует HTTP-клиенты для вызова микросервисов
- Все сервисы запускаются через `docker-compose.yml`
- Настроены зависимости и сетевые подключения между сервисами 